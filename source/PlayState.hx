	...
	// if (SONG.song == 'South')
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
                ...
                // if u are searching for the dialogue thingies, i made at lua cuz i don't wanna search for 10 years here
...
']);
	...
	{
		lightningFlash.animation.play('Flash Lights');
	...
	*/
	...
	override function beatHit()
	{
		...
		/*
		if (curStage == 'forest-rain' && FlxG.random.bool(10) && curBeat > lightningStrikeBeat + lightningOffset)
		{
			lightningStrikeShit();
		}
		lastBeatHit = curBeat;
				...
				switch(achievementName)
				{
					case 'bemtevi_nomiss':
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'DIFICIL' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							...
							{
								case 'bemtevi':
									if(achievementName == 'bemtevi_nomiss') unlock = true;
							}
						}
					case 'aafolou_nomiss':
